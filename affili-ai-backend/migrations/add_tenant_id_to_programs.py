"""
Migration script to add tenant_id to programs table.
...
def migrate():
    """Add tenant_id column to programs table."""
    ...
    try:
        ...
        if "tenant_id" in columns:
            print("✓ Column 'tenant_id' already exists in programs table. Skipping migration.")
            return
        
        # Add tenant_id column with default
        print("Adding 'tenant_id' column to programs table...")
        cursor.execute(f"""
        ...
        print("Setting default tenant for existing programs...")
        cursor.execute(f"""
            UPDATE programs 
