"""
Migration script to add tenant_id to tasks table.
...
def migrate():
    ...
    try:
        ...
        if "tenant_id" in columns:
            print("✓ Column 'tenant_id' already exists in tasks table. Skipping migration.")
            return
        
        # Add tenant_id column with default
        print("Adding 'tenant_id' column to tasks table...")
        cursor.execute(f"""
            ALTER TABLE tasks 
        ...
        print("Setting default tenant for existing tasks...")
        cursor.execute(f"""
            UPDATE tasks 
